...
def process_args(args, defaults, description):
    ...
    parser.add_argument('--test', dest="test",
                        ...
                        help=('Whether to use certain numbers of epochs. ' +
                            'to test and print each step'))
...
def readData():
	...
	episode_No = 0
	for d in dirs: 
		state_filename = 'data/'+d+'/states.npy'
		state_data = np.load(state_filename)
                if episode_No == 0:
                    print state_filename
                    print state_data.shape
		episode[episode_No] = state_data
		episode_No += 1
	
	return episode, len(dirs)
 ...
 #           ale.setBool('sound', False) # Sound doesn't work on OSX
  ...
  #  num_actions = len(ale.getMinimalActionSet())
    ...
    if parameters.nn_file is None:
        network = q_network.DeepQLearner(26,
                                         34,
                                         num_actions,
    ...
    agent = dialogue_agent.NeuralAgent(network,
    ...
    experiment = dialogue_experiment.ALEExperiment( agent,
											  data,
											  state_act_prob,
                                              26,
                                              34,
