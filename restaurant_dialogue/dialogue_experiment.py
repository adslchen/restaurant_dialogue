...
class ALEExperiment(object):
    def __init__(self, agent, data, state_act_p, resized_width, resized_height,
                 resize_method, test, num_epochs, epoch_length, test_length,
                 frame_skip, death_ends_episode, max_start_nullops, rng):
#        self.ale = ale
        self.agent = agent
        self.data = data
        self.num_epochs = num_epochs
        self.epoch_length = epoch_length ##num_of_dirs
        self.test_length = test_length
        self.state_act_p = state_act_p
        self.test= test
    ...
    def run_epoch(self, epoch, num_steps, testing=False):
	 ...
	 #   while steps_left > 0:
        ...
        for e in range(num_steps):
            ...
            logging.info(prefix + " epoch: " + str(epoch) + " num_episodes: " +
                         str(e))
			#  _, num_steps = self.run_episode(steps_left, testing)
            _, num_steps = self.run_episode(self.data[e], testing)
  ...
  #              random_actions = self.rng.randint(0, self.max_start_nullops+1)
   #             for _ in range(random_actions):
    ...
    def _act(self, action):
        ...
        self.buffer_count += 1
        return reward
    def myReward(self, state, action, testing, IDtoState):

        w = 0.5
        totalSlot = 4
        fillingSlot = 0
 #       if(np.sum(state[:91])>0): fillingSlot += 1
  #      if(np.sum(state[92:97])>0): fillingSlot += 1
       # if(np.sum(state[98:211])>0): fillingSlot += 1
       # if(np.sum(state[212:215])>0): fillingSlot += 1
       # CR = fillingSlot*1.0/totalSlot
        DR = 0 
        rstate = np.reshape(state, (884))

   #     stateKey = tuple(map(tuple,state))
        stateKey = toTuple(rstate)
        #print len(stateKey)
        #print stateKey
        if stateKey in self.state_act_p.keys():
        #    print ("yes")
            if testing:
                print ("ID_num: %d" %IDtoState.index(stateKey))
            DR = self.state_act_p[stateKey][action]
        #else :
        #    print("no")
        reward = DR
...
##may not need to skip frames
    ...
    def run_episode(self, states_data, testing):
 ...
 #       start_lives = self.ale.lives()
        ...
        last_state = states_data[:,0]
        last_state = np.reshape(last_state,(34,26))
        action = self.agent.start_episode(last_state)
        num_steps = 0
        if(testing):
            print ("start: action choose: %d"%(action))
        for s in range(1,states_data.shape[1]):			
#        while True:
		   # reward = self._step(action)
		   	reward = self.myReward(last_state,action, testing, IDtoState)  ##last state
		  #  self.terminal_lol = (self.death_ends_episode and not testing)
		   ...
		   # num_steps += 1
			...
			#    break
			if s == states_data.shape[1]-1:
				self.agent.end_episode(reward,True)
                        last_state = states_data[:,s]
                        last_state = np.reshape(last_state,(34,26))
			action = self.agent.step(reward, last_state)
